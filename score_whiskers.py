...
def load_whiskers():
    ...
    # set up the path
    whiskers_path = 'data/whiskers'
    whiskers_dir = os.listdir(whiskers_path)
    # iterate over images
    for image in whiskers_dir:
        # load the image
        image_path = os.path.join(whiskers_path, image) # changed whiskers_dir to whiskers_path
        img = cv2.imread(image_path)
        
        # preprocess the image
        if (img is not None):
            x_whiskers.append(preprocess_image(img))

    x_whiskers = np.array(x_whiskers)
    x_whiskers = x_whiskers.reshape(-1, INPUT_SHAPE_X, INPUT_SHAPE_Y, 1)
    y_whiskers = np.array(y_whiskers)
    
    print("Done.\n")
    return x_whiskers, y_whiskers
...
def vgg16_model():
    ...
    # load labels
    print(len(x_whiskers))
    labels = load_labels()
    y_whiskers = labels.iloc [:, 1]
    y_whiskers = np.array(y_whiskers)
    ...
    x_train, y_train, x_val, y_val, x_test, y_test = split_data(x_whiskers, y_whiskers)
...
def whiskers_resnet():
    # get the data
    x_whiskers, y_whiskers = load_whiskers()
